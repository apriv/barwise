...
```
┌──────────────────────────────────────────────┬───────────────┐
│  Session: 2024-03-15 (RTH, 78 bars)          │  Label Panel  │
│  ◀ prev   Mar 15 ▾    next ▶                 │               │
├──────────────────────────────────────────────┤  [Bar #23]    │
│                                              │               │
...
│  2024-03-15   RTH    78    12       2h ago         │
│  2024-03-14   RTH    78    78 ✓     yesterday      │
│  2024-03-13   RTH    78    0        —              │
│  ...                                               │
└────────────────────────────────────────────────────┘
```
...
1. **Drop file** — 拖文件或选文件
2. **Preview & detect** — 显示前 10 行、识别到的列、时区/session 类型选择
3. **Validate** — 显示行数、session 数、潜在问题（重复时间戳、断点）
4. **Confirm** — 写库进度条；完成跳 `/sessions`
