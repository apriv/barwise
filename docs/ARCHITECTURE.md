...
│   ├── data/
│   │   └── local-es.ts           # 读取本地 ES CSV，自动同步 RTH session
│   ├── export/
...
│   └── samples/es_5m.csv         # 本地 ES 5m 数据源
│
└── docs/                         # 本目录
```
...
**默认用 Server Action**（标签增删改是表单提交或按钮事件）。
...
Server Action 抛错会冒到最近的 `error.tsx`。主页面后续配 `error.tsx`。
...
URL /sessions/123 → Server Component
  → lib/data/local-es.ts.ensureLocalEsRthData() 同步本地 CSV
