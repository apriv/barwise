...
| 能力 | 说明 |
|---|---|
| 读取 K 线 | 直接读取 `data/samples/es_5m.csv`，同步到本地 SQLite |
| 显示图表 | 只显示 ES RTH，每个交易日一张图 |
| 选择 K 线 | 点击单根 bar |
| 打标签 | M2 先做单根 bar 的简单标签；M4 扩展到 Bar / Segment / Context |
| 查看 | 列出已标注内容，可跳回原图 |
| 导出 | 导出 JSONL，用于后续分析或训练 |

**明确不做：** 上传向导、复杂字典管理、CSV zip、多用户、登录、AI 建议、相似 case 检索、实时数据、移动端、回放模式。这些都在 [`ROADMAP.md`](./ROADMAP.md) 的"未来"里。
...
标签字典**会演化**。M2 只要求简单可用；M3 根据 [`LABEL_DICTIONARY.md`](./LABEL_DICTIONARY.md) 完善正式标签体系、显示分组和导出格式；M4 再实现 segment/context 标注。
