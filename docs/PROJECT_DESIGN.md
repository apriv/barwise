...
| 选择 K 线 | 点击单根 bar |
| 打标签 | 右侧栏只做 Bar Label（`bar_quality` / `bar_role`） |
| 查看 | 列出已标注的 bar，可跳回原图 |
| 导出 | 导出 JSONL（bars + bar labels join）用于后续分析或训练 |

**明确不做：** 上传向导、segment/context 标注、字典管理、CSV zip、多用户、登录、AI 建议、相似 case 检索、实时数据、移动端、回放模式。这些都在 [`ROADMAP.md`](./ROADMAP.md) 的"未来"里。
